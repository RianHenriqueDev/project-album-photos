...
export default function AlbumsFilter({albums,loading,className,...props}: AlbumsFilterProps) {
    ...
    return <div className={cx("flex items-center gap-3.5 overflow-x-auto",className)} {...props}>
        ...
        <div className="flex gap-3">
            <Button size="sm" className="cursor-pointer" variant="primary">Todos</Button>
            {!loading ? albums.map((album)=> (
                <>
                <Button key={album.id} size="sm" className="cursor-pointer" variant="ghost">{album.title}</Button>
                 </>
